...
<a href="https://readme-typing-svg.demolab.com">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=20&pause=1200&color=58A6FF&center=true&vCenter=true&width=700&lines=Platform+Engineer+%26+SRE;Kubernetes+%7C+Observability+%7C+Cloud+Architecture;I+fix+systems%2C+not+just+symptoms;Building+platforms+devs+forget+exist" alt="Typing SVG" />
</a>
...
what I do:
  - Build and run Kubernetes platforms for multiple teams
  - Design reliability into systems -- SLOs, error budgets, chaos testing
  - Make observability actually useful -- signal over noise
  - Secure cloud-native infrastructure from the ground up
  - Dig into Linux when Kubernetes is not the real problem
...
| What | How | Result |
|:---|:---|:---:|
| No visibility into service health | Built SLO stack with Prometheus + Grafana from scratch | MTTD 30 min down to 6 |
| Too much alert noise | Multi-window burn rate alerting, RED dashboards | Noise down 60% |
| 70% of incidents, same root cause | One shared backoff-with-jitter library | Fixed 8 services, pattern gone |
| Infra taking 2 days to provision | Reusable Terraform modules, adopted by every team voluntarily | Down to 30 minutes |
| No policy enforcement at deploy time | OPA/Gatekeeper at admission + CI gates | Zero misconfigs in production |
| Untested recovery plans | Quarterly game days with Velero-based DR | RTO/RPO actually validated |
| Cloud costs creeping up | Right-sizing + per-team cost attribution | Down 30% |

---

## On Linux

Kubernetes runs on Linux. When things break at the container boundary, that is where I go.

`strace` for syscall tracing. `perf` for CPU hot paths. `cgroups` and `/proc` for what the kernel actually sees. `tcpdump` and `ss` before touching any manifest. `vmstat` and `smaps` for memory before any application change.

I managed bare Linux systems in production before containers were the default. That background makes Kubernetes debugging faster -- I know where to look before reaching for `kubectl`.
...
[![MSc](https://img.shields.io/badge/MSc-Cloud%20Computing-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white)](https://www.ncirl.ie/)
